...
class ItemRules:
    def evaluate(self, facts: ValidationFacts) -> tuple[ValidationCheck, ...]:
        ...
        if not request.item_pipeline_enabled:
            checks.append(
                _check("ITEM_CONTRACT", ValidationStatus.SKIPPED, "Item extraction was disabled.")
            )
        ...
        if not request.item_pipeline_enabled:
            checks.append(
                _check("ITEMS_PRESENT", ValidationStatus.SKIPPED, "Item extraction was disabled.")
            )
        ...
        if facts.duplicate_groups:
            checks.append(
                _check(
                    ...
                    details={
                        "item_index_groups": [list(value) for value in facts.duplicate_groups]
                    },
                )
            )
        ...
        if not request.item_pipeline_enabled:
            ...
            if request_money_close(request, facts.item_sum, facts.final_total):
                ...
                difference_equals_vat = facts.vat_amount is not None and request_money_close(
                    request, abs(direct_difference), facts.vat_amount
                )
                message = (

"""Ollama implementation of the provider-neutral embedding gateway."""
...
class OllamaEmbeddingGateway:
    """Small, testable adapter for Ollama's ``POST /api/embed`` endpoint."""
    ...
    def embed(self, texts: Sequence[str]) -> EmbeddingBatchResult:
        ...
        with self._lock:
            if self._closed:
                raise RuntimeError("Ollama embedding gateway is closed.")
            try:
        ...
        if not isinstance(raw_vectors, list):
            raise EmbeddingProviderError("Ollama response contains no embeddings array.")
        ...
        if raw_vectors:
            ...
            if not isinstance(first_vector, list):
                raise EmbeddingProviderError("Each embedding must be a numeric array.")
        ...
        try:
            ...
            raise EmbeddingProviderError(f"Ollama returned invalid embeddings: {exc}") from exc
    ...
    def __enter__(self) -> OllamaEmbeddingGateway:
        return self
...
EmbeddingClientError = EmbeddingProviderError
OllamaEmbeddingClient = OllamaEmbeddingGateway

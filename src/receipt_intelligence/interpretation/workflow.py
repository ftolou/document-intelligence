...
class _GeneratedInterpretation(ContractModel):
    ...
    page_interpretations: tuple[PageInterpretation, ...] = Field(max_length=MAX_COLLECTION_SIZE)
...
- Account for every page exactly once using page_interpretations. Use interpreted, blank, irrelevant,
  unreadable, or unprocessed_review_required without treating those states as interchangeable.
...
- Evidence must anchor to interpreted pages. Treat every visual excerpt as model-observed metadata by
  setting excerpt_provenance to model_observed; it is not independently verified source text.
"""

...
class OnePassDocumentInterpreter:
    ...
    def interpret(
        ...
        """Return one typed interpretation with its deterministic validation."""
...
{request.source.source_id!r}. Page numbers and inclusive range endpoints in evidence are one-based
and must remain within the {page_count} supplied page(s).
...
- Account for every page exactly once using page_coverage. Keep interpreted, blank, irrelevant,
  unreadable, and unprocessed states distinct.
...
- Evidence must anchor to interpreted pages. Mark every visual excerpt as model-observed by setting
  excerpt_provenance to model_observed; it is not independently verified source text.
"""

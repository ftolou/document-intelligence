...
def test_interprets_all_outputs_through_one_provider_neutral_call(tmp_path: Path) -> None:
    ...
    assert outcome.validation.status is ValidationStatus.REVIEW_REQUIRED
...
def test_accepts_unsupported_output_without_extracted_assertions(tmp_path: Path) -> None:
    ...
    assert outcome.validation.status is ValidationStatus.VALID
...
def test_rejects_evidence_page_outside_normalized_source(tmp_path: Path) -> None:
    ...
    assert outcome.validation.status is ValidationStatus.INVALID
    assert {issue.code for issue in outcome.validation.issues} >= {
        ValidationIssueCode.EVIDENCE_PAGE_OUT_OF_BOUNDS
    }
    assert [signal.code for signal in outcome.interpretation.review_signals] == ["ambiguous_value"]


def test_preserves_maximum_model_signals_when_page_coverage_is_missing(tmp_path: Path) -> None:
    response = _unsupported_response()
    response["page_interpretations"] = []
    response["review_signals"] = [
        {
    ...
    assert [signal.code for signal in outcome.interpretation.review_signals] == [
        f"model-signal-{index}" for index in range(MAX_COLLECTION_SIZE)
    ]

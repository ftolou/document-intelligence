...
def _response() -> dict[str, object]:
    return {
        "page_observations": [
            {"page": {"page_number": 1}, "state": "processed", "model_observed_text": "12,?"}
        ],
        "classification": {
            "status": "classified",
            "dimensions": [
...
def _unsupported_response() -> dict[str, object]:
    return {
        "page_observations": [{"page": {"page_number": 1}, "state": "irrelevant"}],
...
def test_accepts_unselected_optional_classification_dimension(tmp_path: Path) -> None:
    ...
    assert len(gateway.requests) == 1
    assert result.interpretation.classification.dimensions[0].evidence_refs == ("e-1",)
    assert result.interpretation.classification.dimensions[1].option_paths == ()
    assert result.interpretation.classification.dimensions[1].evidence_refs == ()
...
def test_marks_candidate_fact_not_requested_by_caller_invalid(tmp_path: Path) -> None:
    ...
    assert (
        ...
        is InterpretationValidationFindingCode.UNREQUESTED_PREDICATE
    )
...
def test_accepts_unsupported_output_without_extracted_assertions(tmp_path: Path) -> None:
    ...
    assert result.validation.status is InterpretationValidationStatus.VALID
...
def test_marks_evidence_page_outside_normalized_source_invalid(tmp_path: Path) -> None:
    ...
    assert result.validation.status is InterpretationValidationStatus.INVALID
    assert any(
        finding.code is InterpretationValidationFindingCode.EVIDENCE_PAGE_OUT_OF_RANGE
...
def test_marks_extracted_assertions_without_evidence_invalid(
    ...
    assert result.validation.status is InterpretationValidationStatus.INVALID
    assert any(
        finding.code is InterpretationValidationFindingCode.MISSING_ASSERTION_EVIDENCE
        for finding in result.validation.findings
    )
